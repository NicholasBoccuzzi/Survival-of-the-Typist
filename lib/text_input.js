...
class TextInput {
  ...
  handleHighScoreInput(e) {
    ...
    if(!alphabet.includes(e.key) && e.key !== "Enter" && e.key !== "Backspace") {
      e.preventDefault();
    }
    ...
    if (e.currentTarget.value.length === 3
      ...
      this.gameView.database.ref("scores/SotT").push({score: {
        initials: initials,
        score: score
      }});
    ...
    if ((e.currentTarget.value + e.key).length > 3 && alphabet.includes(e.key)) {
      let initials = e.currentTarget.value.slice(0, 2) + e.key;
