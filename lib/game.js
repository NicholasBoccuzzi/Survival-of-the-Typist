...
class Game {
  ...
  generateEnemies() {
    ...
    this.newEnemies = setInterval(() => {this.createEnemy(); console.log("newEnemiesCalled"); }, this.enemyRate());
  }
  ...
  checkForActiveMovement() {
    if (!this.textInputActive) {
      Object.keys(this.avatar[0].activePowers).forEach((key) => {
        if (this.avatar[0].activePowers[key]) {
          this.avatar[0].power(this.avatar[0].MOVES[key]);
        }
      });
    }
  }
